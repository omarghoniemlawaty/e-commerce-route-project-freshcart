...
const CategoriesList = ({ products , listProducts, setListProducts }) => {
 ...
 const { addToWishlist, removeToWishlist, wishlistProduct } =
  ...
  const [selected, setSelected] = React.useState(listProducts[0]?.category.name || "");
  ...
  // For wishlist animation
  const [wishlistAnim, setWishlistAnim] = useState({});

  const handleWishlistClick = (productId, cb) => {
    ...
    <Col md={12} className="my-5">
      <div className="d-flex flex-wrap">
        <Col md={4} sm={12} xs={12}>
          <button
            className={`${Style["category-btn"]} p-3 w-100${selected === "Women's Fashion" ? " active" : ""}`}
            onClick={e => handleCategoryClick(e, "Women's Fashion")}
          >
            Women's Fashion
          </button>
        </Col>
        <Col md={4} sm={6} xs={12}>
          <button
            className={`${Style["category-btn"]} p-3 w-100${selected === "Men's Fashion" ? " active" : ""}`}
            onClick={e => handleCategoryClick(e, "Men's Fashion")}
          >
            Men's Fashion
          </button>
        </Col>
        <Col md={4} sm={6} xs={12}>
          <button
            className={`${Style["category-btn"]} p-3 w-100${selected === "Electronics" ? " active" : ""}`}
      ...
      <HomeSlider key={animateKey} slidesToShow={4} lg={3} md={2} sm={1} speed={4000}>
        {listProducts.length !== 0 &&
          listProducts.map((product) => (
            <div className={Style["slide-in-left"] + " w-100 mb-5"} key={product.id}>
              <div className="p-4">
