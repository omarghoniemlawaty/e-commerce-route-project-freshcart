...
const Wishlist = () => {
  ...
  // Sync localWishlist with context responsive on mount or when responsive changes
  useEffect(() => {
    if (responsive?.data?.data) {
      setLocalWishlist(responsive.data.data);
    }
  }, [responsive]);
                ...
                {localWishlist.length > 0 ? (
                  localWishlist.map((product) => (
                    <Fragment key={product.id}>
                      <div
                        className={`w-25 mt-1 ${deleting[product.id] ? Style.fadeOut : ''}`}
                      ...
                      <div className={`${Style.updateCountItem} me-4`}>
                        <ul className="d-flex justify-content-between align-items-center h-100">
                          <li>
                            <button
                              className={`mt-1 buyBtn wishBtn py-1 mb-3 m-auto border-0 ${Style.btnAnim}`}
                              onClick={() => {
